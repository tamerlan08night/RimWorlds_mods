...
namespace AKE.endfield
{
    ...
    {
        static OriginiumDrill_Initializer_MKI()
        {
            ...
            ThingDef drillDef = DefDatabase<ThingDef>.GetNamed("OE_OriginiumDrill_MKI", false);
            if (drillDef != null)
            {
                ...
                if (!drillDef.placeWorkers.Contains(typeof(PlaceWorker_DrillAutonomousRadius)))
                {
                    drillDef.placeWorkers.Add(typeof(PlaceWorker_DrillAutonomousRadius));
                }
            }
        }
    }

    public class PlaceWorker_ShowRadiusMKI : PlaceWorker
    {
        public override void DrawGhost(ThingDef def, IntVec3 center, Rot4 rot, Color ghostCol, Thing thing = null)
        {
            GenDraw.DrawRadiusRing(center, 3f);
        }
    }

    public class PlaceWorker_DrillAutonomousRadius : PlaceWorker
    {
        public override void DrawGhost(ThingDef def, IntVec3 center, Rot4 rot, Color ghostCol, Thing thing = null)
        {
            GenDraw.DrawRadiusRing(center, 3f);
    ...
    public class Building_OriginiumDrill : Building
    {
        private const int TransferInterval = 1200;
        private const int EmergencyEjectMax = 50;
        private const float MiningProgressPerTick = 1f / 2200f;

        // МАКСИМАЛЬНА МІСТКІСТЬ ВНУТРІШНЬОГО СХОВИЩА БУРА
        ...
        private void ExecuteAutonomousMining()
        {
            ...
            if (foundResource != null && targetCell.IsValid)
            {
                ...
                int finalYield = Mathf.RoundToInt(yieldCount * 1.10f);
