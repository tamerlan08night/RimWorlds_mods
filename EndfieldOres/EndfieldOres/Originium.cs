...
namespace AKEndfield
{
    ...
    public static class OriginiumPatch
    {
        static OriginiumPatch()
        {
            var harmony = new Harmony("com.akendfield.originiumpatch");
            ...
            var originiumDef = DefDatabase<ThingDef>.GetNamed("OE_OriginiumSurface", true);
            ...
            var currentOreDef = DefDatabase<ThingDef>.GetNamed("OE_OriginiumSurface", false);
            ...
            if (preciousLump != null && currentOreDef != null)
            {
                ...
                if (!preciousLump.mineables.Contains(currentOreDef))
                {
                    ...
                    Log.Message($"[AKEndfield] {currentOreDef.defName} successfully added to long-range scanner.ORIGINIUM");
                }
            }
            else
            {
                Log.Warning("[AKEndfield] Failed to add ore to scanner: def or genStep is null.ORIGINIUM");
            }
        }
    }
}