...
namespace AKEndfield
{
    ...
    public static class FeriumPatch
    {
        static FeriumPatch()
        {
            var harmony = new Harmony("com.akendfield.feriumpatch");
            ...
            var originiumDef = DefDatabase<ThingDef>.GetNamed("OE_FeriumSurface", true);
            ...
            var currentOreDef = DefDatabase<ThingDef>.GetNamed("OE_FeriumSurface", false);
            ...
            if (preciousLump != null && currentOreDef != null)
            {
                ...
                if (!preciousLump.mineables.Contains(currentOreDef))
                {
                    ...
                    Log.Message($"[AKEndfield] {currentOreDef.defName} successfully added to long-range scanner.");
                }
            }
            else
            {
                Log.Warning("[AKEndfield] Failed to add ore to scanner: def or genStep is null.");
            }
        }
    }
}